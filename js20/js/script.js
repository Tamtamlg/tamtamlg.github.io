...
// 1. получаем массив скилов
  var skills = _.map(data, 'skills'); // получаем массив, содержащий массивы скилов
  skills = _.flatten(skills); // извлекаем скилы в один массив
//  console.log(skills);
//  console.log(skills.length);
  skills = _.uniq(skills); // удаляем дубликаты скилов
//  console.log(skills);
//  console.log(skills.length);
  

  function anyCase(a, b) { // для сортировки без учета регистра
  ...
  skills = skills.sort(anyCase); // сортируем
  console.log('skills', skills);
  
  
  // 2. сортируем по количеству друзей
  var names = _.sortBy(data, function (obj) { // сортируем по количеству друзей
    return obj.friends.length;
  });
//console.log(names);
  
  names = _.map(names, 'name'); // получаем имена
  console.log('names', names);
  
  
  
// 3. получаем массив друзей
  var friends = _.map(data, 'friends'); // получаем массив, содержащий объекты друзей + id
  friends = _.flatten(friends); // извлекаем в один массив
  friends = _.map(friends, 'name'); // получаем массив, содержащий имена друзей
//  console.log(friends);
//  console.log(friends.length);
  friends = _.uniq(friends); // удаляем дубликаты друзей
